...
Invest-Scout can also run as a **fully static site** on GitHub Pages, with signal data updated daily via GitHub Actions.

### How It Works

1. **GitHub Actions** runs daily after market close (5pm ET)
2. Python scanner analyzes ~100 popular tickers
3. Results are committed as JSON to the `data/` folder
4. Static HTML/JS frontend loads the JSON and renders everything client-side
5. User watchlists are stored in browser localStorage

### Setup Instructions

1. **Fork or clone this repository**

2. **Make the repository public** (required for free GitHub Pages):
...
4. **Run the initial data generation** (or wait for the scheduled workflow):
   - Go to Actions → "Daily Signal Scan"
...
- **Signal Overview**: View bullish/bearish/neutral signals for ~100 tickers
- **Personal Watchlist**: Add/remove tickers (stored in your browser)
- **Ticker Detail**: Interactive charts (candlestick, RSI, MACD)
- **Backtesting**: Run historical simulations entirely in your browser
- **Daily Updates**: Data refreshes automatically via GitHub Actions

### Customizing the Ticker List

Edit `config/master_tickers.yaml` to change which tickers are scanned:

```yaml
tickers:
  - symbol: AAPL
  - symbol: MSFT
  - symbol: YOUR_TICKER_HERE
```

After pushing changes, the next scheduled run (or manual trigger) will include the new tickers.

### File Structure (Static Site)

```
invest-scout/
├── index.html           # Signal Overview page
├── ticker.html          # Ticker detail page
├── backtest.html        # Backtesting page
├── css/
│   └── styles.css       # Stylesheet
├── js/
│   ├── watchlist.js     # localStorage management
│   ├── signals.js       # Table rendering
│   ├── charts.js        # Plotly chart rendering
│   └── backtest.js      # Client-side backtest engine
├── data/                # Generated by GitHub Actions
│   ├── signals.json     # All ticker signals
│   ├── meta.json        # Metadata (last update, ticker list)
│   └── tickers/         # Per-ticker detail files
│       ├── AAPL.json
│       ├── MSFT.json
│       └── ...
├── .github/workflows/
│   └── scan.yml         # Daily scan workflow
├── scripts/
